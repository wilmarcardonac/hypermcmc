...
subroutine read_table2_LEEUWEN(path_to_datafile)
    ...
    Integer :: stat
    character(len=*) :: path_to_datafile
    Real*8,allocatable,dimension(:) :: LK,pi
    ...
    allocate (FieldHipp(1:arrays_dimension),logP(1:arrays_dimension),Mw(1:arrays_dimension),&
         sigmaMw(1:arrays_dimension),LK(1:arrays_dimension),pi(1:arrays_dimension),&
         muMw(1:arrays_dimension),stat=status1)
    ...
    Do p=1,arrays_dimension
        ...
        read(11,*) FieldHipp(p),logP(p),Mw(p),sigmaMw(p),LK(p),pi(p)

        muMw(p) = 5.d0*log10(1.d-3/pi(p)) + 25.d0 
...
function log_R11_likelihood_W_MW_sigma_int_bw(mu0j,M_w,bw,H0,Zw,av,acal,sigma_int,sigma_int_MW)    !    EQUATION (4) IN R09

    use arrays
    use fiducial
    Implicit none

    Real*8 :: log_R11_likelihood_W_MW_sigma_int_bw,M_w,bw,H0,Zw,av,acal,sigma_int_MW,normalizationA
    Real*8,dimension(4 + 1) :: mu0j,sigma_int
!    Real*8,dimension(number_of_hosts_galaxies) :: sigma_int
    Integer*4 :: m,index_host,number_cepheid

    log_R11_likelihood_W_MW_sigma_int_bw = 0.d0
       ...
       Do m=1,size(Name)
          ...
          Else
             ...
             If (Period(m) .lt. cepheid_Period_limit) then
                ...
                log_R11_likelihood_W_MW_sigma_int_bw = log(new_chi2(chi2R11_W_LMC_E14(mu0j(5),M_w,bw,&
                     Zw,sigma_int(5),m))) + &
                     log(N_tilde_R11_W_LMC(sigma_int(5),m)) + log_R11_likelihood_W_MW_sigma_int_bw
    ...
    Else
       ...
       Do m=1,size(Name)
          ...
          If (Period(m) .lt. cepheid_Period_limit) then
             ...
             log_R11_likelihood_W_MW_sigma_int_bw = -chi2R11_W_LMC_E14(mu0j(5),M_w,bw,Zw,sigma_int(5),m)/2.d0 + &
                  log(N_tilde_R11_W_LMC(sigma_int(5),m)) + log_R11_likelihood_W_MW_sigma_int_bw
    ...
    If (use_HP_per_host) then  ! R11 SAMPLE
       ...
       If (using_jeffreys_prior) then
          ...
          Do index_host=1,number_of_hosts_galaxies-5
             ...
             Do m=1, size(Field)
                ...
                If (host_low_bw(index_host) .eq. Field(m)) then
             ...
             log_R11_likelihood_W_MW_sigma_int_bw = - dble(number_cepheid)*log(chi2R11_W_host_E14(mu0j(index_host),&
                  M_w,bw,Zw,sigma_int(index_host),index_host))/2.d0 - normalizationA/2.d0 + log_R11_likelihood_W_MW_sigma_int_bw
                ...
                If (host_low_bw(index_host) .eq. Field(m)) then
                   ...
                   Else
                      ...
                      If (PeriodR11(m) .lt. cepheid_Period_limit) then
                         ...
                         log_R11_likelihood_W_MW_sigma_int_bw = log(new_chi2(chi2R11_W_E14(mu0j(index_host),M_w,bw,&
                              Zw,sigma_int(index_host),m))) + log(N_tilde_R11_W(sigma_int(index_host),m)) + &
                              log_R11_likelihood_W_MW_sigma_int_bw
             ...
             Do index_host=1,number_of_hosts_galaxies-5
                ...
                If (host_low_bw(index_host) .eq. Field(m)) then
                   ...
                   If (PeriodR11(m) .lt. cepheid_Period_limit) then
                      ...
                      log_R11_likelihood_W_MW_sigma_int_bw = -chi2R11_W_E14(mu0j(index_host),M_w,bw,Zw,&
                           sigma_int(index_host),m)/2.d0 + &
                           log(N_tilde_R11_W(sigma_int(index_host),m)) + log_R11_likelihood_W_MW_sigma_int_bw
       ...
       Do m=1,size(FieldHipp)
          ...
          Else
             ...
             If (10**(logP(m)) .lt. cepheid_Period_limit) then
                ...
                log_R11_likelihood_W_MW_sigma_int_bw = log(new_chi2(chi2R11_W_MW(M_w,bw,Zw,sigma_int_MW,m))) + &
                     log(N_tilde_R11_W_MW(sigma_int_MW,m)) + log_R11_likelihood_W_MW_sigma_int_bw
    ...
    Else
       ...
       Do m=1,size(FieldHipp)
          ...
          If (10**(logP(m)) .lt. cepheid_Period_limit) then
             ...
             log_R11_likelihood_W_MW_sigma_int_bw = -chi2R11_W_MW(M_w,bw,Zw,sigma_int_MW,m)/2.d0 + &
                  log(N_tilde_R11_W_MW(sigma_int_MW,m)) + log_R11_likelihood_W_MW_sigma_int_bw
    ...
    Do m=1,number_of_hosts_galaxies-6 ! SN Ia

       Do index_host=1,number_of_hosts_galaxies-1 ! SN Ia

          If (host_low_bw(m) .eq. host(index_host)) then

             If (use_HP_in_SNIa) then

                log_R11_likelihood_W_MW_sigma_int_bw = log(new_chi2(chi2R11_SNIa(mu0j(m),H0,av,index_host))) + &
                     log(N_tilde_R11_SNIa(index_host)) + log_R11_likelihood_W_MW_sigma_int_bw

             Else

                log_R11_likelihood_W_MW_sigma_int_bw = -chi2R11_SNIa(mu0j(m),H0,av,index_host)/2.d0 + &
                     log(N_tilde_R11_SNIa(index_host)) + log_R11_likelihood_W_MW_sigma_int_bw
    ...
    If (use_HP_in_av) then ! a_v AND a_cal PARAMETERS
       ...
       log_R11_likelihood_W_MW_sigma_int_bw = log(new_chi2((a_v - av)**2/sigma_a_v**2)) - log(2.d0*Pi*sigma_a_v**2)/2.d0  + &
            log(new_chi2((a_cal - acal)**2/sigma_a_cal**2)) - log(2.d0*Pi*sigma_a_cal**2)/2.d0 + &
            log_R11_likelihood_W_MW_sigma_int_bw

    Else

       log_R11_likelihood_W_MW_sigma_int_bw = -((a_v - av)**2/sigma_a_v**2 + log(2.d0*Pi*sigma_a_v**2) )/2.d0  - &
            ((a_cal - acal)**2/sigma_a_cal**2 + log(2.d0*Pi*sigma_a_cal**2) )/2.d0 + log_R11_likelihood_W_MW_sigma_int_bw

    End If
        
    If (use_prior_on_Zw) then

       log_R11_likelihood_W_MW_sigma_int_bw = -((Zw - prior_Zw)**2/sigma_Zw_prior**2 + log(2.d0*Pi*sigma_Zw_prior**2) )/2.d0  +&
            log_R11_likelihood_W_MW_sigma_int_bw
    ...
    If (use_prior_on_bw) then
       ...
       log_R11_likelihood_W_MW_sigma_int_bw = -((bw - prior_bw_from_LMC)**2/sigma_bw_prior**2 + &
            log(2.d0*Pi*sigma_bw_prior**2) )/2.d0  + log_R11_likelihood_W_MW_sigma_int_bw
    ...
    If ( abs(log_R11_likelihood_W_MW_sigma_int_bw) .ge. 0.d0 ) then

       continue

    Else 

       log_R11_likelihood_W_MW_sigma_int_bw = -1.d10

    End If

end function log_R11_likelihood_W_MW_sigma_int_bw
                    ...
                    If (sigma_int_per_R11_host) then
                       ...
                       If (include_all_R11_hosts) then
                          ...
                          Else
                             ...
                             print *,'WRONG NUMBER OF MODEL PARAMETERS (MUST BE 27). CHECK FIDUCIAL MODULE'
                       ...
                       Else
                          ...
                          If (number_model_parameters .eq. 17) then
                             ...
                             Covguess(1,1) = sigma_mu3**2 ! n3982

                             Covguess(2,2) = sigma_mu4**2 ! n3370

                             Covguess(3,3) = sigma_mu5**2 ! n3021

                             Covguess(4,4) = sigma_mu9**2 ! n4258

                             Covguess(5,5) = sigma_mu10**2 ! LMC

                             Covguess(6,6) = sigma_Mw**2 

                             Covguess(7,7) = sigma_bw**2 

                             Covguess(8,8) = sigma_H0**2 

                             Covguess(9,9) = sigma_Zw**2 

                             Covguess(10,10) = sigma_a_v**2

                             Covguess(11,11) = sigma_a_cal**2

                             Covguess(12,12) = sigma_sigma_int**2

                             Covguess(13,13) = sigma_sigma_int**2

                             Covguess(14,14) = sigma_sigma_int**2
                          ...
                          Else
                             ...
                             print *,'WRONG NUMBER OF MODEL PARAMETERS (MUST BE 17). CHECK FIDUCIAL MODULE'
