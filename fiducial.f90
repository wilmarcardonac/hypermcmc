Module fiducial
    ...
    Real*8,parameter    :: prior_mu3 = 31.70d0 ! FROM TABLE 3 IN R11 
    Real*8,parameter    :: prior_mu4 = 32.13d0 ! FROM TABLE 3 IN R11 
    Real*8,parameter    :: prior_mu5 = 32.27d0 ! FROM TABLE 3 IN R11 
    Real*8,parameter    :: prior_mu6 = 32.59d0 ! FROM TABLE 3 IN R11 
    Real*8,parameter    :: prior_mu7 = 31.66d0 ! FROM TABLE 3 IN R11 
    Real*8,parameter    :: prior_mu8 = 31.72d0 ! FROM TABLE 3 IN R11 
    Real*8,parameter    :: prior_mu9 = 24.3d0
    ...
    Real*8,parameter    :: sigma_sigma_int = 0.1d0 ! log10(sigma_int)
    ...
    Real*8,parameter    :: sigma_Zw_prior = 0.02d0 ! 0.02 STRONG PRIOR; 0.25 WEAK PRIOR
    ...
    Integer*4,parameter :: number_model_parameters = 26 ! NUMBER OF PARAMETERS IN MODEL : 3 FOR LMC ALONE WITHOUT METALLICITY, 4 FOR LMC ALONE INCLUDING METALLICITY, 
    ! 10 FOR R11 DATA WITHOUT METALLICITY,
    ! 3 FOR CEPHEIDS ALONE (INCLUDING METALLICITY DEPENDENCE), 12 FOR ALL R11 CEPHEIDS, 14 FOR R11 DATA USING NGC4258 AS AN ANCHOR 
    ! INCLUDING METALLICITY AND REDDENING-FREE MAGNITUDE, 18 FOR ALL R11 CEPHEIDS + LMC CEPHEIDS AND USING LMC AS ANCHOR, 15 FOR ALL R11 CEPHEIDS +
    ...
    ! 16 FOR ALL R11 CEPHEIDS + MW AND LMC AS ANCHORS, 16 FOR ALL R11 CEPHEIDS + NGC4258, LMC AND MW AS ANCHORS, 26 FOR ALL R11 CEPHEIDS + LMC CEPHEIDS 
    !AND USING LMC AS ANCHOR AND SIGMA INT PER R11 HOST
    ...
    Logical,parameter   :: varying_sigma_int = .true.             ! TRUE IF VARYING sigma_int IN MCMC WHEN NO sigma_int_per_host, SET TO FALSE OTHERWISE
